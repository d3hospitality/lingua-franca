...
jobs_obj() { # jobs_obj <bp-concl> <mg-concl>   (use empty string to omit a job)
  local arr="[]"
...
# 1) Happy path: fresh schedule run, success, both required jobs green -> exit 0.
EX="$(run_case "$(run_obj completed success "$FRESH")" "$(jobs_obj success success)")"
expect "fresh green schedule run, both jobs green -> PASS"        "$EX" 0

# 2) The regression this guard EXISTS to catch: cron fired but run failed -> 1.
EX="$(run_case "$(run_obj completed failure "$FRESH")" "$(jobs_obj failure success)")"
...
EX="$(run_case "$(run_obj completed success "$FRESH")" "$(jobs_obj failure success)")"
