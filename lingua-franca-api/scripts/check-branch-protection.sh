#!/usr/bin/env bash
# Branch-protection guard for the `check-alias-guard` required status check.
#
# The topology guard (check-trigger-topology.sh) locks the WORKFLOW YAML so the
# alias-guard job keeps firing exactly once per PR. But it is blind to the OTHER
# half of the gate: branch protection on `main` must list `check-alias-guard` in
# its REQUIRED status contexts. If that context is silently dropped from branch
# protection (an admin toggles it off, a settings sync overwrites it, the rule is
# deleted), the workflow still RUNS on every PR — green checkmark and all — but
# its result no longer BLOCKS merges. The YAML looks healthy; the gate is dead.
# No script in this repo would catch that, because nothing inspects the live
# branch-protection config. This script closes that gap via the GitHub API.
#
# Invariant: repos/<owner>/<repo>/branches/<branch>/protection/required_status_checks
#            .contexts  MUST contain  "check-alias-guard".
#
# Usage:   ./scripts/check-branch-protection.sh
# Env:     REPO    (default d3hospitality/lingua-franca)
#          BRANCH  (default main)
#          CONTEXT (default check-alias-guard)
# Exit:    0 = context is a required check (gate is live)
#          1 = context is ABSENT / no protection / no required checks (gate is DEAD)
...
bold "== check-branch-protection :: is '$CONTEXT' still a required status check on $REPO@$BRANCH? =="
echo ""
...
if [ $RC -ne 0 ]; then
  ...
  if printf '%s' "$ERR" | grep -qiE 'Required status checks not enabled|Branch not protected'; then
    red "  FAIL  $REPO@$BRANCH has NO required status checks — '$CONTEXT' is not gating merges"
    echo ""
    red "== branch-protection gate is DEAD: re-add '$CONTEXT' to required checks on $BRANCH =="
    exit 1
  fi
...
# The reachability/permission check already passed above, so this second call
# only evaluates the contexts array; an exact-string array membership beats a
# substring grep that would false-match e.g. "check-alias-guard-staging".
PRESENT="$(gh api --jq ".contexts | contains([\"$CONTEXT\"])" "$API" 2>/dev/null)"

if [ "$PRESENT" = "true" ]; then
  CONTEXTS="$(gh api --jq '.contexts | join(", ")' "$API" 2>/dev/null)"
  green "  PASS  '$CONTEXT' is a required status check (live required contexts: $CONTEXTS)"
  echo ""
  green "== branch-protection gate is LIVE: '$CONTEXT' still blocks merges on $REPO@$BRANCH =="
  exit 0
else
  CONTEXTS="$(gh api --jq '.contexts | join(", ")' "$API" 2>/dev/null)"
  red "  FAIL  '$CONTEXT' is MISSING from required status checks (present: ${CONTEXTS:-<none>})"
  echo ""
  red "== branch-protection gate is DEAD: '$CONTEXT' was dropped — alias-guard runs but no longer blocks merges =="
  red "   Fix: gh api -X PATCH $API -f 'contexts[]=$CONTEXT'  (or re-add it in repo Settings → Branches)"
  exit 1
fi
